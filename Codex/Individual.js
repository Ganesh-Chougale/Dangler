...
const config = {
  ...
  wantedFiles: []           // Specify the list of files you want to process (can be filenames, relative, or absolute paths)
};
...
function generateSummary(root, selectedDirs) {
  ...
  // Count total files based on wantedFiles
  targets.forEach((dir) => {
    walkDir(dir, (filePath) => {
      ...
      // Process file if it's in the wantedFiles array
      if (
        lang &&
        config.wantedFiles.some((wantedFile) => {
          // Handle different formats of file paths
          const resolvedFilePath = path.isAbsolute(wantedFile)
            ? wantedFile
            : path.resolve(root, wantedFile);
          return resolvedFilePath === path.resolve(root, filePath);
        }) &&
